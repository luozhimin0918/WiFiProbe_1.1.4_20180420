...
public class BitmapUtil {
	...
	private static Bitmap createScaleBitmap(Bitmap src, int dstWidth,
		...
		// 加入到弱引用
		dst = BitmapUtil.getSoftBmpRef(dst);
	...
	public static Bitmap decodeSampledBitmapFromResource(Resources res,
		...
		Bitmap src1 = BitmapFactory.decodeResource(res, resId, options); // 读取图片长长宽
		// 加入到弱引用
		BitmapUtil.getSoftBmpRef(src1);
		...
		// 加入到弱引用
		src = BitmapUtil.getSoftBmpRef(src);
	...
	public static Bitmap decodeSampledBitmapFromFile(String pathName,
		...
		Bitmap src1 = BitmapFactory.decodeFile(pathName, options);
		// 加入到弱引用
		BitmapUtil.getSoftBmpRef(src1);
		options.inSampleSize = calculateInSampleSize(options, reqWidth,
				reqHeight);
	...
	public static Bitmap decodeSampledBitmapFromStream(InputStream is, int reqWidth, int reqHeight){
		final BitmapFactory.Options options = new BitmapFactory.Options();
		options.inJustDecodeBounds = true;
		Bitmap src1 = BitmapFactory.decodeStream(is, null, options);
		// 加入到弱引用
		BitmapUtil.getSoftBmpRef(src1);
		options.inSampleSize = calculateInSampleSize(options, reqWidth,
				reqHeight);
	 ...
	 * @return
	 */
	public static Bitmap getSoftBmpRef(Bitmap bmp){	
        // 加入到软引用
        SoftReference<Bitmap> mReference = new SoftReference<Bitmap>(bmp);
        // 释放硬引用
        bmp = null;
        // 获取软引用中的位图变量
